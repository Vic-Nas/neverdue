...
def index(request):
    try:
        ...
        # No @login_required — public events are accessible to anyone
        def event_detail(request, pk):
            try:
                event = get_object_or_404(Event, pk=pk)
                is_owner = request.user.is_authenticated and request.user == event.user
                if event.private and not is_owner:
                    raise Http404
                ...
                logger.exception("event_detail error pk=%s", pk)
        ...
        def event_edit(request, pk=None):
            try:
                ...
                if request.method == 'POST':
                    ...
                    private = post.get('private') != 'off'  # checkbox: present = public (off), absent = private
                ...
                return render(request, 'dashboard/event_edit.html', {
                    ...
                    'is_owner': True,  # edit page is always owner-only
                })
            except Exception:
